class UsersController < ApplicationController
  before_action :authenticate_user!, only: %i[show edit update destroy recherche_adresse recherche_type recherche_pieces recherche_info carte_ventes]
  ...
  def carte_ventes
    ...
    unless @properties.nil?
      if @properties.size < 10
        new_nb_piece = if params[:nb_piece] != '5+'
                         (params[:nb_piece].to_i + 1).to_s
                       else
                         '4'
                       end
    ...
    @comp.each do |company|
      if company.siege_id.nil? || company.siege == true
        ...
        unless @companies.include?(company_siege)
          get_sells_score(@properties.group_by(&:company), company)
    ...
    current_user.first_company_result_id = if !@companies.at(0).nil?
                                             @companies.at(0).company_name
                                           else
                                             'pas de résultat'
                                           end
    current_user.second_company_result_id = if !@companies.at(1).nil?
                                              @companies.at(1).company_name
                                            else
                                              'pas de résultat'
                                             end
    current_user.third_company_result_id = if !@companies.at(2).nil?
                                             @companies.at(2).company_name
                                           else
                                             'pas de résultat'
                                           end
  ...
  def get_sells_score(properties, company)
    ...
    coef = 0
    total_sell_nbr = 0
    properties.each do |_k, v|
      v.each do |p|
        next unless company.id == p.company_id
        ...
        total_sell_nbr += 1
        total += (date * coefficient)
        coef += coefficient
      end
    end
    company.sells_score = (total / coef) + (total_sell_nbr * 0.5).round
    company.sells_score = 20 if company.sells_score > 20
